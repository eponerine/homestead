...
# Write to InfluxDB
influxDB_writer.write(bucket=influxDB_bucket, record=[influxdb_client.Point("myMeasurement").tag("location", influxDB_location).field("solar_voltage", solar_voltage).field("solar_current", solar_current).field("solar_power", solar_power).field("load_voltage", load_voltage).field("load_current", load_current).field("load_power", load_power).field("battery_voltage",battery_voltage).field("battery_soc",battery_soc).field("battery_temp",battery_temp).field("controller_temp",controller_temp)])